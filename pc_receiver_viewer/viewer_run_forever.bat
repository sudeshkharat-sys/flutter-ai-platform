...
set "LOCK=%SCRIPT_DIR%viewer_watchdog.lock"
...
rem launched its OWN DigitalEyeViewer.exe. They then fight over port 8766:
rem one wins, the rest die instantly with "only one usage of each socket
...
rem viewer_watchdog.log at once, and whichever doesn't hold it fails the
rem redirect.
...
rem stderr is deliberately NOT redirected for the block, so the viewer exe's
...
if not defined GOTLOCK (
    echo.
    echo [watchdog] Another viewer watchdog is already running for this folder.
    echo [watchdog] Not starting a second one -- two of them fight over port 8766
...
:loop
echo [%date% %time%] Starting DigitalEyeViewer.exe >> "%LOG%" 2>nul
"%EXE%"
echo [%date% %time%] DigitalEyeViewer.exe exited ^(code %errorlevel%^) -- restarting in 5s >> "%LOG%" 2>nul
echo.
